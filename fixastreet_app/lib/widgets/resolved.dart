...
class Resolved extends StatefulWidget {
  ...
  @override
  State<Resolved> createState() => _ResolvedState();
}

class _ResolvedState extends State<Resolved> {
  final List<Map<String, dynamic>> resolvedIssues = [];
  Future<List<Map<String, dynamic>>> fetchResolvedIssues() async {
    try {
  ...
  void initState() {
    super.initState();
    fetchResolvedIssues().then((issues) {
      setState(() {
        resolvedIssues.addAll(
          issues.where((issue) => issue['status'] == 'resolved'),
        );
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: resolvedIssues.isEmpty
          ? Center(
              child: CircularProgressIndicator.adaptive(
                valueColor: AlwaysStoppedAnimation<Color>(
                  Colors.green[600]!,
                ), // Use the same color as the app bar
                strokeWidth: 2.0, // Adjust the stroke width as needed
              ),
            )
          : ListView.builder(
              itemCount: resolvedIssues.length, // Example count
