...
class _PendingState extends State<Pending> {
  final List<Map<String, dynamic>> pendingIssues = [];
  Future<List<Map<String, dynamic>>> fetchPendingIssues() async {
    try {
  ...
  void initState() {
    super.initState();
    fetchPendingIssues().then((issues) {
      setState(() {
        pendingIssues.addAll(
          issues.where((issue) => issue['status'] == 'pending'),
        );
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: pendingIssues.isEmpty
          ? Center(
              child: CircularProgressIndicator.adaptive(
                valueColor: AlwaysStoppedAnimation<Color>(
                  Colors.green[600] ?? Colors.green,
                ), // Adaptive spinner color
              ),
            )
          : ListView.builder(
              itemCount: pendingIssues.length, // Example count
